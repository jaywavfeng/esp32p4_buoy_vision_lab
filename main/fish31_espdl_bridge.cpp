...
static const char *TAG = "fish31_bridge";

extern const uint8_t fish31_cls_model_start[] asm("_binary_fish31_cls_model_espdl_start");
extern const uint8_t fish31_cls_model_end[] asm("_binary_fish31_cls_model_espdl_end");

static dl::Model *s_model;
static dl::image::ImagePreprocessor *s_preprocessor;
static SemaphoreHandle_t s_lock;
static uint32_t s_model_input_w = FISH31_INPUT_W;
static uint32_t s_model_input_h = FISH31_INPUT_H;
...
static esp_err_t ensure_model(void)
{
    ...
    if (!s_model) {
        s_model = new (std::nothrow) dl::Model((const char *)fish31_cls_model_start,
        ...
        if (!s_model) {
            ...
            ESP_LOGE(TAG, "failed to allocate Fish31 dl::Model");
            return ESP_ERR_NO_MEM;
        }
        read_model_input_shape();
        ESP_LOGI(TAG, "Fish31 model loaded, input=%" PRIu32 "x%" PRIu32 ", classes=%u, bytes=%" PRIu32,
                 s_model_input_w, s_model_input_h, (unsigned)FISH31_CLASS_COUNT,
                 fish31_espdl_model_bytes());
    }
    ...
    if (!s_preprocessor) {
        ...
        if (!s_preprocessor) {
            xSemaphoreGive(s_lock);
            ESP_LOGE(TAG, "failed to allocate Fish31 preprocessor");
            return ESP_ERR_NO_MEM;
        }
    }
    xSemaphoreGive(s_lock);
...
{
    return (uintptr_t)fish31_cls_model_end > (uintptr_t)fish31_cls_model_start;
}

uint32_t fish31_espdl_model_bytes(void)
{
    return (uint32_t)(fish31_cls_model_end - fish31_cls_model_start);
}
...
static esp_err_t fill_topk(dl::TensorBase *output, fish31_espdl_result_t *out)
{
    if (!output || !out || output->get_size() < (int)FISH31_CLASS_COUNT) {
        return ESP_ERR_INVALID_RESPONSE;
    }

    float logits[FISH31_CLASS_COUNT] = {0};
    float max_logit = -INFINITY;
    for (uint32_t i = 0; i < FISH31_CLASS_COUNT; i++) {
    ...
    float probs[FISH31_CLASS_COUNT] = {0};
    float sum = 0.0f;
    for (uint32_t i = 0; i < FISH31_CLASS_COUNT; i++) {
    ...
    uint32_t order[FISH31_CLASS_COUNT] = {0};
    for (uint32_t i = 0; i < FISH31_CLASS_COUNT; i++) {
        order[i] = i;
    }
    std::sort(order, order + FISH31_CLASS_COUNT, [&](uint32_t a, uint32_t b) {
        return probs[a] > probs[b];
    });

    out->top_k_count = FISH31_TOP_K < FISH31_CLASS_COUNT ? FISH31_TOP_K : FISH31_CLASS_COUNT;
    for (uint32_t i = 0; i < out->top_k_count; i++) {
        ...
        strlcpy(out->top_k[i].label, FISH31_LABELS[cls], sizeof(out->top_k[i].label));
    }
                              ...
                              dl::image::img_t *img)
{
    ...
    case V4L2_PIX_FMT_SBGGR8:
        ...
        break;
    default:
        ESP_LOGW(TAG, "unsupported Fish31 frame pixel format 0x%08" PRIx32, pixel_format);
...
static esp_err_t classify_img_locked(const dl::image::img_t &img, fish31_espdl_result_t *out)
{
                                                ...
                                                fish31_espdl_result_t *out)
{
    ...
    esp_err_t ret = fish31_espdl_classify_frame((const uint8_t *)img.data,
