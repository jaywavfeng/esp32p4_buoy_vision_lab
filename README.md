...
CMake 配置阶段会运行 `tools/apply_component_patches.py`。针对 ESP-IDF 6.0.1，项目补丁修复 FAT 格式化后重新挂载失败却返回成功的问题，并补齐 Hosted SDMMC slot 清理：重新挂载错误会交给应用恢复流程处理，Web 不会误报格式化成功。补丁按版本和源码片段严格匹配；环境不符合预期时构建会停止，禁止静默使用未知 IDF 行为。
