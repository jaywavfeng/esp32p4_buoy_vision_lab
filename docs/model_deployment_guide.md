...
| `yolo11` | 本机训练 Coke/Sprite YOLO11n | `models/yolo11_coke_sprite_416_s8_p4.espdl` 嵌入固件 |
| `yolo26` | 本机训练 Coke/Sprite YOLO26n | `models/yolo26_coke_sprite_raw_heads_416_allint8_p4.espdl` 嵌入固件 |
| `mlp` | 小型手写 MLP | C 头文件 `main/coke_sprite_mlp_model.h` |
...
注意：API 会写 NVS，重启后可能保留上一次选择。如果怀疑状态不对，直接调用上面的 API 切回 `coco`。
