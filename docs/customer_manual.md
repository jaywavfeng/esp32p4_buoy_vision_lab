...
USB 导出不要求先打开 Web。插入 USB 数据线后，设备会暂停采集/录像，把 TF 卡交给电脑。导出完成后须先安全弹出并拔掉 USB 数据线，再从 Web 点击「USB 恢复存储」；恢复和写入验证成功前，TF 卡会保持隔离，不会被板端误用。
...
5. 拔掉 USB 数据线；此时 TF 卡仍保持隔离。
6. 打开 Web，确认 USB 已断开，然后点击「USB 恢复存储」。等待页面显示恢复成功、TF 可用后再开始采集。

USB 导出期间 Web 服务不会主动关闭。若页面可访问，会显示 USB 已插入、TF 被 USB 占用。TinyUSB 的断开提示不等于数据线已物理拔出，所以设备不会自动回收 TF。若恢复失败，页面会保留 TF 隔离状态并显示原因；按提示确认线缆已拔掉后再次点击「USB 恢复存储」。
...
| USB 拔掉后 TF 未恢复 | 这是保护存储的预期状态。确认已安全弹出并拔线，再在 Web 点击「USB 恢复存储」；若失败，按页面错误提示排查后重试。 |
...
- 清空录像记录后列表为空。
- 插入 USB 后电脑出现 `P4_BUOY`，Web 仍在线；安全弹出并拔线后 TF 保持隔离，点击「USB 恢复存储」后才恢复并通过写读验证。
