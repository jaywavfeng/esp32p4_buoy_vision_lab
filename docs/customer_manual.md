...
2. 板子默认同时启动热点和路由器连接：
   - 热点名称：`YOUR_AP_SSID`
   - 热点密码：`YOUR_WIFI_OR_AP_PASSWORD`
   - 固定访问地址：`http://YOUR_AP_IP/`
3. 手机或电脑连接热点后，打开：
   - 实时监控首页：`http://YOUR_AP_IP/`
   - 图片/视频验证页：`http://YOUR_AP_IP/validate`
...
```text
POST http://YOUR_AP_IP/api/storage/remount?confirm=REMOUNT
POST http://YOUR_AP_IP/api/storage/format?confirm=FORMAT
```
...
- 页面打不开：先连接 `YOUR_AP_SSID` 热点，再访问 `http://YOUR_AP_IP/`；如果热点已消失，重启板子。
