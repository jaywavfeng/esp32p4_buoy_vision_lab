...
附件目录 `D:\ZJU\轻量化板端目标识别` 已做部署检查，优先检查模型为：
...
```powershell
cd D:\ZJU\Project\esp32p4_buoy_vision_lab
$env:IDF_COMPONENT_CACHE_PATH="D:\idfcmcache"
$env:ESP_IDF_VERSION="6.0"
idf.py build
idf.py -p COM3 flash
```

本机 ESP-IDF 目录是 `C:\esp\v6.0.1\esp-idf`，但当前 Wi-Fi Remote/ESP-Hosted 组件需要 `ESP_IDF_VERSION=6.0` 才会选择正确配置。
